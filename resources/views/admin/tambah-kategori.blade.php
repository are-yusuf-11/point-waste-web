...
<head>
    ...
    <title>Tambah Kategori Sampah - PointWaste Admin</title>
    ...
    <main class="ml-[260px] min-h-screen flex flex-col">
        <header class="fixed top-0 right-0 w-[calc(100%-260px)] h-16 bg-white border-b border-outline-variant flex items-center justify-between px-8 z-40">
            <div class="flex items-center gap-2 text-secondary font-medium text-sm">
                ...
                <span class="text-on-surface">Tambah Kategori Master</span>
            </div>
        </header>
        ...
        <section class="mt-16 p-8 max-w-[800px] w-full mx-auto flex flex-col gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-primary">Tambah Kategori Sampah Master</h2>
                    <p class="text-sm text-secondary">Definisikan tipe material sampah baru dan nilai konversi poin reward-nya.</p>
                </div>
            </div>

            <div class="bg-white border border-outline-variant rounded-xl shadow-sm overflow-hidden">
                <form action="{{ route('admin.kategori-sampah.store') }}" method="POST" class="p-6 flex flex-col gap-5">
                    @csrf
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <input type="text" id="nama_kategori" name="nama_kategori" value="{{ old('nama_kategori') }}" required
                            placeholder="Contoh: Plastik PET, Kardus Bekas, Kaleng Aluminium"
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="relative flex items-center">
                            <input type="number" id="poin_per_kg" name="poin_per_kg" value="{{ old('poin_per_kg') }}" min="0" required
                        ...
                        <p class="text-xs text-slate-400">Jumlah poin yang diberikan kepada warga untuk setiap 1 kg setoran material ini.</p>
                        @error('poin_per_kg')
                            <p class="text-xs text-error font-medium mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4" 
                            placeholder="Berikan catatan jenis sampah yang termasuk serta kondisinya (misal: botol harus dibilas bersih, kardus harus dilipat kering, dll.)"
                            class="w-full border border-outline-variant rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary @error('deskripsi') border-error @enderror">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="text-xs text-error font-medium mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex flex-col gap-2 pt-2 border-t border-slate-100">
                        <label class="text-sm font-bold text-slate-800">Status Operasional Awal</label>
                        <div class="flex items-center gap-6 mt-1">
                            <label class="flex items-center gap-2 cursor-pointer text-sm font-medium text-slate-700">
                                <input type="radio" name="status_aktif" value="1" checked class="text-primary focus:ring-primary h-4 w-4 border-slate-300">
                                <span class="flex items-center gap-1 text-emerald-700 font-bold bg-emerald-50 px-2.5 py-0.5 border border-emerald-100 rounded-full text-xs">
                                    <span class="material-symbols-outlined text-xs">check_circle</span> Langsung Aktif
                                </span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer text-sm font-medium text-slate-700">
                                <input type="radio" name="status_aktif" value="0" class="text-primary focus:ring-primary h-4 w-4 border-slate-300">
                                <span class="flex items-center gap-1 text-rose-700 font-bold bg-rose-50 px-2.5 py-0.5 border border-rose-100 rounded-full text-xs">
                                    <span class="material-symbols-outlined text-xs">cancel</span> Nonaktif Sementara
                                </span>
                            </label>
                        </div>
                    </div>
