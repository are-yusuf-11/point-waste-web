            ...
            <div class="relative z-10 w-full max-w-md text-on-primary-container text-left px-md">
                ...
                <div class="grid grid-cols-2 gap-md mt-xl">
                    <div class="p-md bg-white/10 backdrop-blur-md rounded-xl border border-white/20 shadow-sm">
                        <span class="block font-headline-md text-headline-md text-white">{{ $totalSampah }} kg</span>
                        <span class="font-label-md text-label-md text-white/80 uppercase tracking-wider text-[11px]">Sampah Terolah</span>
                    </div>
                    <div class="p-md bg-white/10 backdrop-blur-md rounded-xl border border-white/20 shadow-sm">
                        <span class="block font-headline-md text-headline-md text-white">{{ $jumlahWarga }}</span>
        ...
        <section class="w-full lg:w-1/2 flex items-center justify-center p-md lg:p-xl bg-surface relative">
            <div class="w-full max-w-[440px]" id="auth-canvas">
                ...
                <div class="flex bg-secondary-container rounded-lg p-xs mb-lg">
                    <a href="{{ route('showLogin') }}" class="flex-1 py-sm text-center font-label-md text-label-md rounded-md transition-all text-on-secondary-container hover:text-primary">
                        Login
                    </a>
                    <a href="{{ route('showRegister') }}" class="flex-1 py-sm text-center font-label-md text-label-md rounded-md transition-all bg-surface-container-lowest text-primary font-bold shadow-sm">
                        Register
                    </a>
                </div>

                <form class="space-y-lg block" id="register-form" method="POST" action="{{ route('register') }}">
                    @csrf
                    
                    <div class="space-y-sm">
                        <label class="font-label-md text-label-md text-on-surface-variant">Nama</label>
                        <div class="relative">
                            <input class="w-full px-md py-sm bg-surface-container-lowest border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary-fixed focus:border-primary outline-none transition-all placeholder:text-outline/60 text-on-surface" placeholder="Masukan Nama" type="text" name="nama" value="{{ old('nama') }}" required autofocus />
                        </div>
                    ...
                    <div class="space-y-sm">
                        ...
                        <div class="relative">
                            ...
                            <button class="absolute right-md top-1/2 -translate-y-1/2 text-on-surface-variant" onclick="togglePassword('register-password')" type="button">
